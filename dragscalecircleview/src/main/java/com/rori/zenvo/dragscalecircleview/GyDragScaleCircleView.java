 ...
 * @author hpfs0
 */
public class GyDragScaleCircleView extends AppCompatImageView {

    @SuppressWarnings("unused")
    private static final String TAG = GyDragScaleCircleView.class.getName();
    ...
    //                       constructor
    // -------------------------------------------------------------
    public GyDragScaleCircleView(Context context) {
        super(context);
        init(context, null);
    }

    public GyDragScaleCircleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public GyDragScaleCircleView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public Bitmap getCroppedCircleBitmap() {
        ...
        /*croppedRect 现在存储的是 相对于bitmap 的位置 不是originDrawable 的位置*/
    ...
    protected void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        // custom attr
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.GyDragScaleCircleView);
        try {
            mHasGuideLine = typedArray.getBoolean(R.styleable.GyDragScaleCircleView_gyhasGuideLine, true);
            mGuideLineSize = typedArray.getFloat(R.styleable.GyDragScaleCircleView_gyguideLineSize, getResources().getDimension(R.dimen.guideline_width));
            mGuideLineColor = typedArray.getInt(R.styleable.GyDragScaleCircleView_gyguideLineColor, getResources().getColor(R.color.guideline));
            mBorderSize = typedArray.getFloat(R.styleable.GyDragScaleCircleView_gyborderSize, getResources().getDimension(R.dimen.border_width));
            mBorderColor = typedArray.getInt(R.styleable.GyDragScaleCircleView_gyborderColor, getResources().getColor(R.color.border));
        } finally {
            typedArray.recycle();
        }
    ...
    private void initCircleCropWindow(@NonNull RectF bitmapRect) {
        ...
        mCenterPointX = getMeasuredWidth() / 2.0f;
        mCenterPointY = getMeasuredHeight() / 2.0f;
     ...
     */
    float scale = 1;/*把图片 按fitCenter 显示到imageView上面的放大或者缩小倍数*/
    private Drawable originDrawable; /*持有的是最原始的图片的尺寸*/
    private RectF getBitmapRect() {
        ...
        final Rect drawableBounds = originDrawable.getBounds();
        final Bitmap src = ((BitmapDrawable) originDrawable).getBitmap();
        final float imgViewWidth = getMeasuredWidth();
        final float imgHeight = getMeasuredHeight();

        int type = 0;
        /*w1 / h1 w2 / h2*/
        if(drawableBounds.right / drawableBounds.bottom > imgViewWidth / imgHeight ){
            type = 1;
            /*代表图片宽度 放大到跟view一样大小 高度 不会超出view*/
            scale = imgViewWidth / drawableBounds.right;
            final Matrix matrix = new Matrix();
            matrix.postScale(scale, scale);
            Bitmap bitmap = Bitmap.createBitmap(src, 0, 0,src.getWidth(), src.getHeight(), matrix, true);
            setImageBitmap(bitmap);
        }else{
            /*代表图片高度 放大到跟view一样大小 宽度 不会超出view*/
            scale = imgHeight / drawableBounds.right;
            final Matrix matrix = new Matrix();
            matrix.postScale(scale, scale);
            Bitmap bitmap = Bitmap.createBitmap(src, 0, 0,src.getWidth(), src.getHeight(), matrix, true);
            setImageBitmap(bitmap);
        ...
        if(type == 1){
            left =0;
            top = (imgHeight - src.getHeight()*scale)/2;
        }else{
            left = (imgViewWidth - src.getWidth()*scale)/2;
            top = 0;
        }
        final float right = left + drawableDisplayWidth * scale;
        final float bottom = top + drawableDisplayHeight * scale;
    ...
    private void center(float dx, float dy) {
        if ((mCenterPointX + dx - mRadius < mBitmapRect.left)
                || (mCenterPointY + dy - mRadius < mBitmapRect.top)
                ||  (mCenterPointX + dx + mRadius) > mBitmapRect.right
                ||  (mCenterPointY + dy + mRadius) > mBitmapRect.bottom) {
            return;
        }
        mCenterPointX += dx;
        mCenterPointY += dy;
    ...
    private void side(float lastX, float lastY, float rawX, float rawY) {
        ...
        float maxRadius = Math.min(mBitmapRect.width()/2,mBitmapRect.height()/2);
